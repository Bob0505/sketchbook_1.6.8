...
char result;
	...
	data = BMP180_REG_ID;
	result = readBytes(&data, 1);
	if (data != BMP180_REG_ID_VAL) {
		reset();
		delay(1);
		data = BMP180_REG_ID;
		result = readBytes(&data, 1);
...
char SFE_BMP180::reset(void)
	// Sends a soft reset command to BMP180, causing it to perform a power-on reset
	// From datasheet:
	// Soft reset (register E0h): If set to 0xB6, will perform the same sequence as power on reset.
	// returns 1 for success, 0 for fail from I2C comms
{
	unsigned char data[2] = {BMP180_REG_RESET, BMP180_COMMAND_RESET};
	return ( writeBytes(data, 2) );
}

void SFE_BMP180::calcParams(void) {
	double c3,c4,b1;
...
void SFE_BMP180::setCal(int16_t _AC1, int16_t _AC2, int16_t _AC3, int16_t _VB1, int16_t _VB2, int16_t _MB, int16_t _MC, int16_t _MD,
...
char SFE_BMP180::readCal(int16_t &_AC1, int16_t &_AC2, int16_t &_AC3, int16_t &_VB1, int16_t &_VB2, int16_t &_MB, int16_t &_MC, int16_t &_MD,
	uint16_t &_AC4, uint16_t &_AC5, uint16_t &_AC6 ) {
