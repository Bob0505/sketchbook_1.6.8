...
Adafruit_MAX30100 MAX30100 = Adafruit_MAX30100();
...
//  }
  ...
  MAX30100.setSpO2SampleRate(MAX30100_SPO2SR_50HZ);

  max30100_spo2_samplerate_t rate = MAX30100.getSpO2SampleRate();
  uint16_t ratehz = 0;
  switch (rate) {
  ...
  MAX30100.setLEDpulseWidth(MAX30100_LEDPW_1600US);

  max30100_led_pulsewidth_t width = MAX30100.getLEDpulseWidth();
  ...
  MAX30100.setRedLEDcurrent(MAX30100_LEDCURRENT_30_6MA);
  MAX30100.setIRLEDcurrent(MAX30100_LEDCURRENT_11MA);
  max30100_led_current_t red = MAX30100.getRedLEDcurrent();
  max30100_led_current_t ir = MAX30100.getIRLEDcurrent();
...
void loop() {
  ...
  while (databufferptr < MAX_SAMPLES) {
    ...
    // some data ready!
    uint32_t fifodata = MAX30100.readRegister32(MAX30100_FIFO_DATA_REG);
  ...
  while ((databufferptr+16) < MAX_SAMPLES) {
    ...
    MAX30100.readFIFO(fifo_buffer, 8);
  ...
  Serial.println("Data read!");
  //Serial.print("Temperature = "); Serial.println(MAX30100.readTemperature(), 4);
